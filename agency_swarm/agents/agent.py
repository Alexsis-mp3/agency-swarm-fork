...
class Agent:
    ...
    def __init__(
        ...
        Parameters:
            ...
            instructions (str, optional): Path to a file containing specific instructions for the agent, or direct string content. If a file path is provided, the file will be read automatically. Defaults to an empty string.
        ...
        # Auto-detect and read instructions file if needed
        if instructions and self._is_file_path(instructions):
            self.instructions = self._read_instructions_file(instructions)
        else:
        ...
        # self._save_settings()  # Temporarily commented out for testing
    ...
    def _is_file_path(self, instructions: str) -> bool:
        """
        Determine if the instructions parameter is a file path.
        """
        return (
            isinstance(instructions, str) and (
                instructions.endswith((".md", ".txt", ".rst")) or
                "/" in instructions or
                "\\" in instructions or
                instructions.startswith("./") or
                instructions.startswith("../") or
                os.path.exists(instructions)
            )
        )

    def _read_instructions_file(self, file_path: str) -> str:
        """
        Read instructions from a file, with proper error handling.
        """
        try:
            # Handle relative paths relative to the calling file
            ...
            if not content:
                return f"Warning: Instructions file '{file_path}' is empty."
            return content
        except FileNotFoundError:
            error_msg = f"ERROR: Instructions file '{file_path}' not found. Please ensure the file exists and the path is correct."
            print(f"Agency Swarm Warning: {error_msg}")
            return error_msg
        except PermissionError:
            error_msg = f"ERROR: Permission denied reading instructions file '{file_path}'."
            print(f"Agency Swarm Warning: {error_msg}")
            return error_msg
        except UnicodeDecodeError:
            error_msg = f"ERROR: Unable to decode instructions file '{file_path}'. Please ensure it's a valid text file with UTF-8 encoding."
            print(f"Agency Swarm Warning: {error_msg}")
            return error_msg
        except Exception as e:
            error_msg = f"ERROR: Unexpected error reading instructions file '{file_path}': {str(e)}"
            print(f"Agency Swarm Warning: {error_msg}")
            return error_msg
